...
**Amended 2026-09-16 — one shared clock, board-wide re-render.** Every countdown (card chip, job panel, verification ring) reads a `now` prop threaded down from a single `useNow()` 1 s interval rather than owning a timer. `components/dashboard.tsx` calls `useNow()` unconditionally, so the consequence is stronger than "while a claim counts down": the whole board re-renders once per second for the life of the page, claim or no claim. Accepted at demo scale (eight orders); a real board would start the interval only when something is actually counting down, memoise the countdown subtree, or run the animation in CSS. The alternative — a timer per chip — trades the re-render for N unsynchronised intervals whose digits visibly disagree.
